...
class Visualizer:
    def __init__(self, where_to_find_data, session_key='plotimages'):
        # Ensure where_to_find_data is a dictionary
        self.data = where_to_find_data if isinstance(where_to_find_data, dict) else {}
        self.session_key = session_key
        
        # Retrieve or initialize run history from session
        import flask
        self.all_runs_data = flask.session.get(self.session_key, {})
        
        self.figures = {}
        self.run_colors = ['blue', 'red', 'green', 'purple', 'orange', 'brown', 'magenta']
        
        # Safely determine current run number
        try:
            if self.data and list(self.data.keys()):
                first_key = list(self.data.keys())[0]
                self.current_run_number = len(self.all_runs_data.get(first_key, [])) + 1
            else:
    ...
    def return_list_of_plots_for_ressources(self):
        ...
        # Store the current run's data
        for resource, resource_data in self.data.items():
            # Skip if resource_data is empty
            if not resource_data:
                continue

            if resource not in self.all_runs_data:
                self.all_runs_data[resource] = []
            
            # Add current run's data with run number
            self.all_runs_data[resource].append({
                'run_number': self.current_run_number,
                'data': resource_data
            })

        ressources_to_be_plotted = list(self.data.keys())

        for ressource in ressources_to_be_plotted:
            # Ensure the resource has data before generating plot
            if self.all_runs_data.get(ressource):
                plot = self.generate_plot(
                    inputdata=self.all_runs_data[ressource], 
                    ressourcename=ressource
                )
                list_of_plots_for_ressources.append(plot)

        # Update session with new run data
        import flask
        flask.session[self.session_key] = self.all_runs_data

        return list_of_plots_for_ressources
        
    def generate_plot(self, inputdata, ressourcename):
        # Create or retrieve figure for this resource
        if ressourcename not in self.figures:
            self.figures[ressourcename] = plt.figure()
        
        plt.figure(self.figures[ressourcename].number)
        plt.clf()  # Clear previous plot contents

        # Plot all runs for this resource
        for run in inputdata:
            run_number = run['run_number']
            resource_data = run['data']

            if resource_data:
                timestamps = [t[0] for t in resource_data]
                queue_lengths = [t[2] for t in resource_data]
                
                # Select color based on run number
                color = self.run_colors[(run_number - 1) % len(self.run_colors)]
        ...
        # Adjust plot limits
        all_timestamps = [t[0] for run in inputdata for t in run['data']]
        all_queue_lengths = [t[2] for run in inputdata for t in run['data']]
        
        if all_timestamps and all_queue_lengths:
            plt.xlim(0, max(all_timestamps) + 5)
            plt.ylim(0, max(all_queue_lengths) + 5)
...
class SimulationManager:
    ...
    def run_sim_from_config_get_results(self, runconfig:SimulationConfiguration):
        try:
            ...
            # Only run visualization if there's monitoring data
            if self.mysim.monitor and self.mysim.monitor.data_monitoring:
                self.mysim.run_visualization()
            else:
                # If no monitoring data, set plots to empty
            ...
            # Return empty results to prevent breaking the app
            return {"plots": [], "simcore": None}
